// IMPORTANT: Import polyfills FIRST before pdfjs
import './node-polyfills';

// Getting pdfjs to work in Node.js environment
import * as pdfjs from "pdfjs-dist";
import * as path from "path";
import { pathToFileURL } from "url";

// Set worker source to the worker file with proper file:// URL
 ...
 *
 * @param source - Either a file URL string (for browser) or Uint8Array buffer (for Node.js)
 * @example
 * // Browser usage
 ...
 * const textItems = await readPdf(new Uint8Array(buffer));
 */
export const readPdf = async (source: string | Uint8Array): Promise<TextItems> => {
  const pdfFile = await pdfjs.getDocument(source).promise;
